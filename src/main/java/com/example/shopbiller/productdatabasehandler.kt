...
//    }
    ...
    fun readdataproduct():ArrayList<products>{ // this function for read all data from table Bills for make bills
        ...
        if (result.moveToFirst()){
            do {
                ...
                products.productid = result.getString(1)
                products.productname = result.getString(2)
                products.productprice = result.getInt(3)
                list.add(products)
   ...
   fun countproduct():Int{  // this function for count of product in Bills table for make bill
    ...
    fun deleteproduct1(ID: String){
        val db = this.writableDatabase
        db.delete(TABLE_NAME,PRODUCT_ID+ "=" + ID,null)
